...
public class CuentaValidator {
    ...
    public void validate(CuentaDto cuentaDto) throws ClienteDoesntExistException, InputErrorException {
        ...
        if (!"PESOS".equals(cuentaDto.getMoneda().name().toUpperCase()) && !"DOLARES".equals(cuentaDto.getMoneda().name().toUpperCase())) {
            throw new InputErrorException("La MONEDA ingresada no es valida.");
        }
        if (!"CUENTA_CORRIENTE".equals(cuentaDto.getTipoCuenta().name().toUpperCase()) && !"CAJA_AHORRO".equals(cuentaDto.getTipoCuenta().name().toUpperCase())) {
